...
// - 코드를 보시면 main 메소드가 있는 Main 클래스만 public 으로 정의해주고 있고 나머지 클래스는 public 없이 정의하고 있다는 것을 아실 수 있습니다.
        ...
        switch (command) {
            ...
            case 'T':
                // 'T 금액', 활성 계좌에서 비활성 계좌로 금액만큼 이체
            	{
            		...
            		if(account == primary_account){
            		  if (primary_account.withdraw(amount) && secondary_account.deposit(amount)){
            		  ...
            		  if (secondary_account.withdraw(amount) && primary_account.deposit(amount)){
            		    writer.setTransaction("transfer $", amount);
            		  }
            		  else{
            		    writer.setTransaction("transfer error");
            ...
            case 'I':
                // 'I 이율', 활성 계좌의 금액을 이율만큼 증가
        		{
            		...
            		if((interest > 0.0) && (100.0 > interest)){
            		  // interest = Math.floor(interest * 100.0) / 100.0;
