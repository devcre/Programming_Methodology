    ...
    public void processTransactions()
    {
        ...
        switch (command) {
            ...
            case 'T':
            	...
            	{
            		BankAccount inactive = (account == primary_account) ? secondary_account : primary_account; 
            		int amount = reader.readAmount();
            		if (account.withdraw(amount) && inactive.deposit(amount)) {
            ...
            case 'I':
        		...
        		{
            		double interest = reader.readAmount() / 100.0;
            		int amount = (int)((double)(account.getBalance()) * interest);
            		if (account.deposit(amount)) {
            			writer.setTransaction("interest $", amount);
            		}
            		else writer.setTransaction("interest error");
            		break;
            	}
            default:
